...
{
	unsigned int size = (unsigned int)mBindPose.Size();
	mInvBindPose.resize(size);

	for (unsigned int i = 0; i < size; ++i) 
	{
		const Transform& globalJointTransform = mBindPose.GetGlobalTransform(i);
		mInvBindPose[i] = inverse(transformToMat4(globalJointTransform));
	}
}
